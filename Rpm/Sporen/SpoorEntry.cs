...
namespace Rpm.Productie
{
    public class SpoorEntry
    {
        ...
        [ExcludeFromSerialization] public int PerUur { get; set; }
        ...
        public string CreateHtmlText(int aantal, bool checkperuur)
        {
            ...
            else
            {
                ...
                var xrestsporen = xnodig % xsporen;
                if (xnodig < xsporen)
                {
                    xsporen = xnodig;
                    xrestsporen = 0;
                ...
                var xmaken = xnodig * xprodsperlengte;
                ...
                if (PakketAantal > 0)
                {
                    ...
                    var xrestpakket = xnodig > 0 ? xnodig % PakketAantal : 0;
                ...
                if (checkperuur && !string.IsNullOrEmpty(ArtikelNr) && ArtikelNr.Length > 4)
                {
                    ...
                    {
                        var tijd = Math.Round((double) aantal / PerUur, 2);
                        peruur =
                            "<h5>" +
                ...
                var xoverschotmaat = overschot > 0
                    ...
                    : "";
                var xoms = string.IsNullOrEmpty(ProductHtmlOmschrijving)
                    ? string.IsNullOrEmpty(ProductOmschrijving) ? "" : $"<h4>{ProductOmschrijving}</h4>"
                    : ProductHtmlOmschrijving;
                xret = $"<p color='{Color.Navy.Name}'>" +
                       ...
                       $"Je haalt <b>{xprodsperlengte}</b> producten uit <b>{xtotal / 1000} meter</b> met een reststuk van <b>{xrest}mm({Math.Round(xrest / xtotal * 100, 4)}%)</b>.<br>" +
                       // $"</ul>" +
                       "</p>";
            }
        ...
        public bool CompareTo(SpoorEntry verpakking)
        {
            ...
            if (!string.Equals(ArtikelNr, verp.ArtikelNr, StringComparison.CurrentCultureIgnoreCase))
                return false;
            if (!string.Equals(ProductOmschrijving, verp.ProductOmschrijving,
