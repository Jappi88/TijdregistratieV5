...
namespace AutoUpdaterDotNET
{
    ...
    public static class AutoUpdater
    {
        ...
        {
            Start(AppCastURL, true, myAssembly);
        }

        public static void Start(bool onlychecknew, Assembly myAssembly = null)
        {
            Start(AppCastURL, onlychecknew, myAssembly);
        }
        ...
        /// <param name="checknew">Show only if new version is enabled</param>
        /// <param name="myAssembly">Assembly to use for version checking.</param>
        public static void Start(string appCast, bool checknew, Assembly myAssembly = null)
        {
            try
            {
            ...
            if (!Running && _remindLaterTimer == null)
            {
                ...
                if (!_isWinFormsApplication) Application.EnableVisualStyles();
                ...
                if (Synchronous)
                    try
                    {
                        var result = CheckUpdate(assembly, checknew);
                    ...
                    using (var backgroundWorker = new BackgroundWorker())
                    {
                        backgroundWorker.DoWork += (sender, args) =>
                        {
                            ...
                            args.Result = CheckUpdate(mainAssembly, checknew);
                        };
        ...
        private static object CheckUpdate(Assembly mainAssembly, bool onlychecknew)
        {
            ...
            try
            {
                ...
                using (var client = GetWebClient(BaseUri, BasicAuthXML))
                {
                    var xml = client.DownloadString(BaseUri);
                ...
                else
                {
                    ...
                    {
                        var compareResult = DateTime.Compare(DateTime.Now, remindLaterAt.Value);

                        if (compareResult < 0) return remindLaterAt.Value;
                    }
                }
            }
            catch
        ...
        public static void ShowUpdateForm(UpdateInfoEventArgs args)
        {
            ...
            {
                if (UpdateFormSize.HasValue) updateForm.Size = UpdateFormSize.Value;

                if (updateForm.ShowDialog().Equals(DialogResult.OK)) Exit();
            }
        }
