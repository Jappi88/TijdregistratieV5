...
namespace Rpm.Misc
{
    ...
    public class StickyWindow : NativeWindow
    {
        ...
        /// </summary>
        private static readonly ArrayList GlobalStickyWindows = new();
        ...
        {
            var rParent = originalForm.Bounds;
            offsetPoint = point;
        ...
        // General Stuff
        private readonly Form originalForm; // the form
        ...
        private void Resize(Point p)
        {
            ...
            var iRight = formRect.Right;
            var iBottom = formRect.Bottom;
            ...
            if ((resizeDirection & ResizeDir.Left) == ResizeDir.Left)
            {
                ...
                var iNewWidth = formRect.Width + formOffsetRect.Width + formOffsetRect.X;
            ...
            if ((resizeDirection & ResizeDir.Top) == ResizeDir.Top)
            {
                var iNewHeight = formRect.Height + formOffsetRect.Height + formOffsetRect.Y;
        ...
        private void Move_Stick(Rectangle toRect, bool bInsideStick)
        {
            ...
            if (formRect.Bottom >= toRect.Top - stickGap && formRect.Top <= toRect.Bottom + stickGap)
            {
                ...
                if (Math.Abs(formRect.Left + formRect.Width - toRect.Left - toRect.Width) <=
                    Math.Abs(formOffsetPoint.X))
                    // snap right 2 right
            ...
            if (formRect.Right >= toRect.Left - stickGap && formRect.Left <= toRect.Right + stickGap)
            {
                if (bInsideStick)
                {
                    ...
                    if (Math.Abs(formRect.Top + formRect.Height - toRect.Top) <= Math.Abs(formOffsetPoint.Y) &&
                        bInsideStick)
                        // snap Bottom to Top
                ...
                if (Math.Abs(formRect.Top + formRect.Height - toRect.Top - toRect.Height) <=
                    Math.Abs(formOffsetPoint.Y))
                    // bottom 2 bottom
